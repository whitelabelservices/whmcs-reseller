<?php
/**
 * WhiteLabelServices VPS Module - Add Network Interface AJAX Endpoint
 */
...
try {
    ...
    if (!$result || empty($result['status'])) {
        throw new Exception('Failed to add network interface');
    }

    logActivity("WLS Network - Interface added for service {$serviceId} (VM: {$vmId})");
    ...
    logActivity('WLS Network Error - Failed to add interface: ' . $e->getMessage());
    echo json_encode([
        'success' => false,
        'error' => $e->getMessage(),
    ]);
